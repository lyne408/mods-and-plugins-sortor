...
import ModOrganizerTwo from './ModOrganizerTwo'

import SkyrimSpecialEdition from './SkyrimSpecialEdition'
...
import {Empty_Array, Empty_Set, Empty_String} from "./util/ValueUtil";
/*
 ...
 * Unofficial SkyrimSpecialEdition Special Edition Patch.esp
 * *SkyrimSpecialEdition Project Optimization - No Homes - Full Version.esm
 * ```
 *
 *
 * loadorder.txt
 ...
 */
export default class ModOrganizerTwoProfile {


    /**
     * ModManagerTwoProfile 依赖于 ModManagerTwo 工作, 应提供 API 获取
     *
     * 比如 DataFile 列表必须借助 ModManagerTwo 获取,
     *      所有 Mod 列表 也是
     */
    modManagerTwo: ModOrganizerTwo | null = null
    // [lyne408] TS 初始化一个确定类型的空对象. 不能用 modManagerTwo: ModManagerTwo = null
    // [lyne408] 由于历史原因, undefined 和 null 总是共存. 习惯上, 表示空对象用 null, 表示无值用 undefined.

    /**
     * MO profile 内文本文件的首行一般是这个
     * @type {string}
     */
    static readonly File_Tip = `# This file was automatically generated by Mod Organizer.`

    /**
     * fileTip 所占行数
     * @type {number}
     */
    static readonly File_Tip_Line_Count: number = 1
     ...
     * @type {string[]}
     */
    static readonly Vanilla_DLC_Mods_Reversed: Array<string> = [
        `*DLC: Dragonborn`,
        `*DLC: HearthFires`,
        `*DLC: Dawnguard`
    ]
     ...
     * @type {string[]}
     */
    static readonly Vanilla_ESM_Loadorder: Array<string> = [
        'Skyrim.esm',
        'Update.esm',
        'Dawnguard.esm',
        'HearthFires.esm',
    ...
    static readonly Plugins_File_Name = 'plugins.txt'
    static readonly Loadorder_File_Name = 'loadorder.txt'
    static readonly Modlist_File_Name = 'modlist.txt'
     ...
     * </dependencies>
     */
    modDirectory: string = Empty_String
     ...
     * </dependencies>
     */
    profileDirectory: string = Empty_String
     ...
     * </dependencies>
     */
    allModsSorted: Array<string> = Empty_Array
     ...
     * @type {string[]}
     */
    enabledThirdPartyModsOrdered: Array<string> = Empty_Array
     ...
     * @type {string[]}
     */
    thirdPartyPluginsSorted: Array<string> = Empty_Array
     ...
     * @type {Set<string>}
     */
    enabledThirdPartyPluginsSet: Set<string> = Empty_Set
    ...
    async init({
        ...
        if (Array.isArray(allModsSorted)) {
            ...
            this.allModsSorted = await ModOrganizerTwo.getModsSortedSortedByName(this.modDirectory)
        }
    }
    ...
    async sortPlugins(isBackup: boolean = true): Promise<void> {
        const pluginsFilePath = path.resolve(this.profileDirectory, ModOrganizerTwoProfile.Plugins_File_Name)
        const pluginsFileLines = [ModOrganizerTwoProfile.File_Tip]
        ...
        return writeArrayToFile({
            ...
            isBackup,
            onFinish: () => {
                console.log('[Info]', `This ${ModOrganizerTwoProfile.Plugins_File_Name} has been sorted!`)
            }
        })
    }
    ...
    async sortLoadorder(isBackup: boolean = true): Promise<void> {
        const loadorderFilePath = path.resolve(this.profileDirectory, ModOrganizerTwoProfile.Loadorder_File_Name)
        ...
        const loadorderFileLines = [ModOrganizerTwoProfile.File_Tip, ...ModOrganizerTwoProfile.Vanilla_ESM_Loadorder]
        return writeArrayToFile({
            filePath: loadorderFilePath,
            array: loadorderFileLines.concat(this.thirdPartyPluginsSorted),
            isBackup,
            onFinish: () => {
                console.log('[Info]', `This ${ModOrganizerTwoProfile.Loadorder_File_Name} has been sorted!`)
            }
        })
    }
    ...
    async sortModlist(isBackup: boolean = true): Promise<void> {
        const modlistFilePath = path.resolve(this.profileDirectory, ModOrganizerTwoProfile.Modlist_File_Name)
        const modlistFileLines: Array<string> = [ModOrganizerTwoProfile.File_Tip]
        ...
        modlistFileLines.push(...ModOrganizerTwoProfile.Vanilla_DLC_Mods_Reversed)
        ...
        return writeArrayToFile({
            ...
            isBackup,
            onFinish: () => {
                console.log('[Info]', `This ${ModOrganizerTwoProfile.Modlist_File_Name} has been sorted!`)
            }
        })
    ...
    async sort({
        ...
        if (isSortPlugins) {
            console.info(`[Info]`, `Sort plugins by mod's priority!`)
    ...
    private async initPluginsRelativeProperty(isSortsModsByName: boolean = false) {
        const pluginsOfEnableMods: Array<string> = []
        ...
        for await (const modName of this.enabledThirdPartyModsOrdered) {
            const plugins = (await filterFiles({
                ...
                extnames: SkyrimSpecialEdition.Data_File_Extnames
            }))
            pluginsOfEnableMods.push(...plugins)
        }
        // 去除重复, 保留最后位置, 大小写不敏感
        this.thirdPartyPluginsSorted = removeStringArrayDuplication(pluginsOfEnableMods, 'last', false)
        const pluginsFilePath = path.join(this.profileDirectory, ModOrganizerTwoProfile.Plugins_File_Name)
        ...
        // 存在 plugins.txt
            ...
            // 若有第三方 plugin
            if (pluginLines.length > ModOrganizerTwoProfile.File_Tip_Line_Count) {
                for (let i = ModOrganizerTwoProfile.File_Tip_Line_Count; i < pluginLines.length; i++) {
    ...
    private async initModsRelativeProperty() {
        ...
        // 存在 modlist.txt
            ...
            // 可能存在启用的第三方 mod 们
            if (modsInModlistFile.length > ModOrganizerTwoProfile.File_Tip_Line_Count) {
                ...
                for (let i = modsInModlistFile.length - 1; i >= ModOrganizerTwoProfile.File_Tip_Line_Count; i--) {
