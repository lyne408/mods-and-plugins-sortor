...
import {extnameOf, fileNameContainsOf, fileNameHasSubstrings, getFileName, getFolderName, safenPath,} from './PathUtil'
import {containsOf, equalsOf, hasSubstrings, isNonEmptyString} from './StringUtil'
import {CRLF_String, Empty_String} from "./ValueUtil";
...
export async function isEmptyDirectory({
    ...
    if (isCheckIsDirectory) {
        if (await isDirectory(pathArg, isCheckExistence)) {
...
export async function createDirectory({
    ...
    const isDirectoryVar = await isDirectory(pathArg, false)
    ...
    async function mkdirLocal(): Promise<boolean> {
        return fsPromises.mkdir(pathArg, {recursive: true})
            ...
            .catch((error: Error) => {
                // `Error: Create directory ${pathArg} failed!`,
 ...
 * createFile
 * @param pathArg
 * @param isCheckExistence
 * @param isOverwiteFile
 * @param isOverwriteEmptyDirectory
 * @param isOverwriteNotEmptyDirectory
 */
...
export type FilterFilesAndDirectoriesParameter = {
    ...
    /**
     * 是否从完整的路径中查询
     * 比如从 .NET Core 3.1.10 和 .NET Core 5.0.1 的 runtime 目录中查询 5.0.1 版本的 WindowsBase.dll
     *
     * <necessity optional />
     *
     * <value value="true">
 ...
 * @param {string} comparedDir
 * @param {boolean} isSameExtname 是否比较扩展名
 */
export async function reserveSmallerFiles({
                                              reservedDir,
                                              comparedDir,
                                              isSameExtname = true
                                          }: ReserveSmallerFileParameter): Promise<void> {
    ...
    */
    ...
    if (isSameExtname) {
        ...
        async function initMapLocal(map: Map<string, Array<string>>, iterable: Iterable<unknown>) {
            for await (const file of iterable) {
                const fileName = getFileName(file + Empty_String)
                const extname = path.extname(file + Empty_String)
                if (map.has(fileName)) {
                    // @ts-ignore
...
export async function writeArrayToFile({
    ...
    // [lyne408] unknown 转为 string: varName + Empty_String
    const stringArray: Array<string> = array.map((element: unknown) => element + Empty_String)
    let str: string
